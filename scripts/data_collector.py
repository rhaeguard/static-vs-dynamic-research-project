...
def project_to_tuple(p: Project):
    return (p.name, p.lang, p.metric_results.sqale_debt_ratio, p.metric_results.ncloc)
...
def get_all_projects() -> Tuple[int, List[Project]]:
    ...
    projects = list(map(create_project, response['components']))
...
def set_metrics_of_project(project: Project):
    ...
    """
    url = BASE_URL + "/measures/component?metricKeys=sqale_debt_ratio,ncloc&component="+project.key
    response = requests.get(url, auth=HTTPBasicAuth(*credentials))
    response = response.json()
...
def populate_all():
    ...
    for p in projects:
        print("%-40s %-10s %6.2f %8d" % project_to_tuple(p))
    return projects
